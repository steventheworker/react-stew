...
helpers.ajax = function(url, settings = {}) {
    function urlify(url, method, data) {
        let dataString = '';
        for (let key in data) dataString += key + "=" + escape(data) + "&";
...
class FrankenQuery {
    constructor(selector) {
        ...
        if (typeof selector === "function") return helpers.documentReady(selector);
        let el, res = helpers.quickQuery(selector); //test for simple queries: ex: helpers('#id'), helpers('.class'), helpers('tag')
        if (!res && (el = helpers.element(selector))) {
        ...
        res = res || helpers.queryAll(selector) || [];
        this.selector = selector;
        this.isQuery = true;
        this.__proto__.toString = () => '[FrankenQuery Object]';
        helpers.setResults(this, res);
    }
    add(query) { //combine two queries results
        ...
        return helpers.changeResults(this, res);
    }
    ...
    find(selector) {
        let res = [];
        this.each((i, el) => {
            Array.from(helpers.query(selector, el)).forEach((el, i) => res.push(el));
        });
        return helpers.changeResults(this, res);
    }
    ...
    prev() {
        ...
        return helpers.changeResults(this, prevs.filter(el => el !== null));
    }
    next() {
        let nexts = [];
        this.each((i, el) => nexts.push(el.nextElementSibling));
        return helpers.changeResults(this, nexts.filter(el => el !== null));
    }
    index(tar) {
        ...
        tar = helpers.element(tar); //find index from tar in query
        if (!tar) tar = (helpers.query(tar) || "")[0];
        if (tar) for (let i in this.res) if (this.res[i] === tar) return Number(i);
        return -1;
    }
    first() {return helpers.changeResults(this, [this.get(0)]);}
    last() {return helpers.changeResults(this, [this.get(this.length - 1)]);}
    clone() {
        let clones = [];
        this.each((i, el) => clones.push(el.cloneNode(true)));
        return helpers.changeResults(this, clones);
    }
    remove() {return this.each((i, el) => el.parentNode.removeChild(el));}
    replaceWith(data) {
        let outer = (helpers.element(data).outerHTML || data);
    ...
    parent() {
        ...
        return helpers.changeResults(this, parents);
    }
    children() {
        let children = [];
        this.each((i, el) => Array.from(el ? el.children : []).forEach(child => children.push(child)));
        return helpers.changeResults(this, children);
    }
    siblings() {
        ...
        return helpers.changeResults(this, siblings);
    }
    empty() {return this.each((i, el) => el.innerHTML = "");}
    appendTo(selector) {
        let el;
        if (typeof selector === "string") el = helpers(selector)[0]; else el = helpers.element(selector);
        if (!el) el = helpers.query(selector)[0];
        return this.each((i, cur) => el.appendChild(cur));
    }
    append(data) {
        data = helpers.element(data);
        if (!data) return this;
    ...
    after(data, before) {
        data = helpers.element(data).innerHTML;
    ...
    hasClass(tarClass) {
        ...
        this.each((i, el) => {
            ...
            else if (new RegExp('(^| )' + tarClass + '( |helpers)', 'gi').test(el.className)) has = true;
        });
        return has;
    }
    addClass(tarClass) {return this.each((i, el) => el.className += ' ' + tarClass);}
    removeClass(tarClass) {return this.each((i, el) => el.className = el.className.replace(new RegExp('(^|\\b)' + tarClass.split(' ').join('|') + '(\\b|helpers)', 'gi'), ' '));}
    ...
    fadeIn(t, callback) {
        ...
        this.css({
            visibility: 'visible',
            opacity: 1,
            transition: `opacity helpers{t}ms ease-in-out`,
        }, function() {
    ...
    one(...a) {return helpers.ev(this, a, "one");}
    on(...a) {return helpers.ev(this, a, "on");}
    off(...a) {return helpers.ev(this, a, "off");}
    focus(...a) {return helpers.ev(this, a, "focus");}
    blur(...a) {return helpers.ev(this, a, "blur");}
    click(...a) {return helpers.ev(this, a, "click");}
    dblclick(...a) {return helpers.ev(this, a, "dblclick");}
    mousedown(...a) {return helpers.ev(this, a, "mousedown");}
    mouseup(...a) {return helpers.ev(this, a, "mouseup");}
    mouseenter(...a) {return helpers.ev(this, a, "mouseenter");}
    mouseleave(...a) {return helpers.ev(this, a, "mouseleave");}
    keypress(...a) {return helpers.ev(this, a, "keypress");}
    keydown(...a) {return helpers.ev(this, a, "keydown");}
    keyup(...a) {return helpers.ev(this, a, "keyup");}
    change(...a) {return helpers.ev(this, a, "change");}
    resize(...a) {return helpers.ev(this, a, "resize");}
    load(...a) {return helpers.ev(this, a, "load");}
    unload(...a) {return helpers.ev(this, a, "unload");}
    scroll(...a) {return helpers.ev(this, a, "scroll");}
    submit(...a) {return helpers.ev(this, a, "submit");}
    trigger(ev) {
    ...
    event() {
        ...
        if (["on", "off", "one"].indexOf(ev) !== -1) {
            ...
            evts = evts.split(" ");
            this.each((i, el) => evts.forEach(evt => helpers[ev === "off" ? "off" : "on"](el, evt, arg[2], callback, arg[4], (ev === "one"))));
        } else { //listener w/o selector (ie.click() .focus()) OR trigger()
            if (!arg[1]) { //trigger
                this.trigger(ev);
            } else this.each((i, el) => helpers.on(el, ev, undefined, arg[1], arg[2]));
        }
        return this;
    }
}
